...
const DATABASE_ROLES = {
  ...
  3: {
    name: "AccessAdmin",
    description: "The user can modify access to the database for other users",
  },
  4: {
    name: "BackupOperator",
    description: "The user can backup the database",
  },
  5: {
    name: "DdlAdmin",
    description:
  ...
  9: {
    name: "DenyDataReader",
    description: "The user cannot select from any table in the database.",
  },
} as const;
  ...
  function deleteDatabaseUser(id: number) {
    ...
    Promise.all(rolePromises)
      .then(() => {
        ...
        return api.databaseUser.deleteDatabaseUserDelete({
          databaseUserId: id,
        });
      })
      .then((response) => {
    ...
    <>
      <div className="mb-4">
        ...
        <p className="lead text-muted">
          Manage database users and their role assignments across your
          databases. SQL Runner uses these users to enforce role-based access
          control, ensuring each user has the appropriate permissions for their
          tasks. This is essential for maintaining security and proper access
          control in a multi-tenant environment.
        </p>
        <Button
          variant="primary"
                  ...
                  <td className="px-4 py-3">
                    <div className="d-flex flex-wrap gap-2">
                      ...
                      {(!databaseUser.databases ||
                        databaseUser.databases.length === 0) && (
                    ...
                    <div className="d-flex flex-wrap gap-2">
                      {databaseUser.roles?.map((role) => (
                        <Badge
                          ...
                          className="px-3 py-2"
                          title={
                            DATABASE_ROLES[role as keyof typeof DATABASE_ROLES]
                              ?.description
                          }
                        >
                          {DATABASE_ROLES[role as keyof typeof DATABASE_ROLES]
...
function AddDatabaseUserModal({
  ...
  const roles = Object.entries(DATABASE_ROLES).map(
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const deleteRolePromises = (user.roles || []).map(
          (role: DatabaseRole) =>
            api.databaseUserRole.deleteDatabaseUserRoleDelete({
              databaseUserId: user.id!,
              databaseRoleId: role,
        ...
        // Add new roles
        const addRolePromises = values.roles.map((roleId: string) =>
          api.databaseUserRole.addDatabaseUserRoleCreate({
            databaseUserId: user.id!,
            databaseRoleId: parseInt(roleId) as DatabaseRole,
          }),
